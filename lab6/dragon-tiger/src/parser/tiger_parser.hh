...
switch (yytype)
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  tiger_parser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
...
#if 201103L <= YY_CPLUSPLUS
  inline
  tiger_parser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  tiger_parser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}
  ...
  inline
  int
  tiger_parser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }
